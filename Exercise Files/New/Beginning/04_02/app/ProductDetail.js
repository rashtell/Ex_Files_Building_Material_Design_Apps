...
class FabDialogCombo extends React.PureComponent {
  ...
  constructor(props) {
    ...
    this._dialogRevealProgress = new Animated.Value(this.state.dialogVisible ? 1 : 0);
  }
  componentDidUpdate(prevProps, prevState) {
    if (this.state.dialogVisible !== prevState.dialogVisible) {
      Animated.timing(this._dialogRevealProgress, {
        toValue: this.state.dialogVisible ? 1 : 0,
        duration: 300,
      }).start(() => {
        if (this.state.dialogVisible === false) this.setState({ containerOrigin: null });
      });
    }
  }
  render() {
    const onFabPress = async () => {
      ...
      this.setState({ dialogVisible: true, containerOrigin });
    };
    const onBackgroundPress = () => this.setState({ dialogVisible: false });
    const AnimatedFab = Animated.createAnimatedComponent(Fab);
    const animatedModalStyle = {
      elevation: metrics.elevation.appBar + 1,
      backgroundColor: this._dialogRevealProgress.interpolate({
        inputRange: [0, 1],
        outputRange: ['rgba(0,0,0,0)', 'rgba(0,0,0,0.5)'],
      }),
      transform: [
        {
          translateY: this._dialogRevealProgress.interpolate({
            inputRange: [0, 0.1, 1],
    ...
    const dialogStyleOnReveal = getDialogStyleOnReveal(this._dialogRevealProgress);
    const origin = this.state.containerOrigin;
    const fabContainerStyleOnReveal = getFabContainerStyleOnReveal(this._dialogRevealProgress, origin);
    const fabContainerStyle = [styles.fabContainer, this.props.style, fabContainerStyleOnReveal];
    return (
      <View style={[styles.fullScreen, { justifyContent: 'center' }]}>
        ...
        <Animated.View style={fabContainerStyle}
          pointerEvents="box-none" ref={v => this._fabContainer = v}>
          <Animated.View style={[styles.dialog, dialogStyleOnReveal]}>
          ...
          <AnimatedFab icon='add-shopping-cart' color={FAB_COLOR}
            style={[styles.fab, getFabStyleOnReveal(this._dialogRevealProgress)]}
            onPress={onFabPress} />
        </Animated.View>
      </View>
    );
...
//////////// Reveal dialog functions /////////

// Reveal: move (with slight zoom) ==> zoom ==> fade

const moveEnd = 0.2;
const zoomEnd = 0.7;
const sizeRatioWhenMoveEnds = 1.5;

const getFabStyleOnReveal = (progress: Animated.Value) => {
  const opacity = progress.interpolate({
    inputRange: [0, zoomEnd, 1],
    outputRange: [1, 1, 0],
  });
  const sizeWhenMoveEnds = FAB_SIZE * sizeRatioWhenMoveEnds;
  const maxSize = Math.sqrt(Math.pow(DIALOG_WIDTH / 2, 2) + Math.pow(DIALOG_HEIGHT / 2, 2)) * 2;

  const computePos = (fabSize, containerWidth, containerHeight) => ({
    left: (containerWidth - fabSize) / 2,
    top: (containerHeight - fabSize) / 2,
  });

  const initialPos = computePos(FAB_SIZE, FAB_CONTAINER_INIT_SIZE, FAB_CONTAINER_INIT_SIZE);
  const containerSizeWhenMoveEnds = FAB_CONTAINER_INIT_SIZE * sizeRatioWhenMoveEnds;
  const posWhenMoveEnds = computePos(sizeWhenMoveEnds, containerSizeWhenMoveEnds, containerSizeWhenMoveEnds);
  const posWhenZoomEnds = computePos(maxSize, DIALOG_WIDTH, DIALOG_HEIGHT);

  // left and top keep the FAB at the center of the fab container
  const inputRange = [0, moveEnd, zoomEnd, 1];
  const left = progress.interpolate({
    inputRange,
    outputRange: [initialPos.left, posWhenMoveEnds.left, posWhenZoomEnds.left, posWhenZoomEnds.left],
  });
  const top = progress.interpolate({
    inputRange,
    outputRange: [initialPos.top, posWhenMoveEnds.top, posWhenZoomEnds.top, posWhenZoomEnds.top],
  });
...
const getDialogStyleOnReveal = (progress: Animated.Value) => {
  const inputRange = [0, 0.7, 1];
  const opacity = progress.interpolate({
    inputRange,
    outputRange: [0, 0, 1],
  });
...
const getFabContainerStyleOnReveal = (progress: Animated.Value, origin) => {
  const inputRange = [0, moveEnd, zoomEnd, 1];
  const sizeWhenMoveEnds = FAB_CONTAINER_INIT_SIZE * sizeRatioWhenMoveEnds;
