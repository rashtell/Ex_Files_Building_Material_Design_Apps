...
class FabDialogCombo extends React.PureComponent {
  ...
  render() {
    const onFabPress = async () => {
      const metrics = await measureInWindow(this._fabContainer);
      this.setState({ dialogActivated: true, containerOrigin: metrics });
    };
...
// TODO exercise
// Change the path of fab container from a straight line to an arc
